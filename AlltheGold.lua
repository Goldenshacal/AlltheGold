...
if type(pc_data.last_update) ~= "number" then
	err("realm = %s, pc = %s, pc_data.last_update = %s", realm, pc, pc_data.last_update)
							...
							if need_last_login then
								if pc == AlltheGold.pc then
									-- The PC is loged in
									col_text[col_no] = C:Yellow(L["Logged in"])
								else
									col_text[col_no] = pc_data.last_update and 
															 pc_data.last_update ~= 0 and 
															 (L["|cff00ffffSeen |cffffffff%s |cff00ffffago|r"]):format(AlltheGold:FormatTime(time() - pc_data.last_update)) 
															 or ""
								end
								col_align[col_no] = 'CENTER'
								col_no = col_no + 1
							...
							line, column = tooltip:AddLine()
							local color = (i % 2) == 1 and .3 or nil
	...
	if self:GetOption('show_coins') then
		line, column = tooltip:AddLine()
		tooltip:SetCell(line, 1, C:Gold( L["Total Gold Value: "] ), "LEFT", nb_columns - 1)
...
function AlltheGold:OnTimePlayedMsg(msg, seconds_played)
...
function AlltheGold:EventHandlerOnlySort(msg)
...
function AlltheGold:EventHandlerWithSort(msg)
...
function AlltheGold:EventHandler(msg)
...
-- Event handler for the other events registered
function AlltheGold:EventHandlerHonorGain()
...
function AlltheGold:DetectFactionChange()
...
function AlltheGold:SaveVar()
	 --self:Debug("AlltheGold:SaveVar()")
	 ...
	 --self:Print("AlltheGold:SaveVar() Zone: ->%s<- ->%s<-", GetZoneText(), self.db.global.data[self.faction][self.realm][self.pc].zone_text)
...
function AlltheGold:SaveVarHonor()
...
function AlltheGold:SetSecondsPlayed(seconds_played)
...
function AlltheGold:GetOption( option, ... )
	--self:Debug(format("AlltheGold:GetOption(%s) = %s", option or 'nil', tostring(self.db.profile.options[option] or 'nil')))
...
-- Set an option value
function AlltheGold:SetOption( option, value, ... )
	--self:Debug(format("AlltheGold:SetOption(%s): old %s, new %s", option or 'nil', tostring(self.db.profile.options[option] or 'nil'), tostring(value or 'nil') ))
	...
	elseif option == 'show_minimap_icon' then
		if value then
			...
			ldbicon:Show("AlltheGold")
		else
			self.db.profile.options.ldbicon.hide = true
			ldbicon:Hide("AlltheGold")
		end

		ldbicon:Refresh("AlltheGold", self.db.profile.options.ldbicon)
...
function AlltheGold:EstimateTimePlayed( pc, realm, time_played, last_update )
...
-- was in an Inn
function AlltheGold:EstimateRestedXP( pc, realm, level, rested_xp, max_rested_xp, last_update, is_resting, curent_xp )
	 --self:Debug("AlltheGold:EstimateRestedXP: %s, %s, %s, %s, %s, %s, %s",pc, realm, level, rested_xp, max_rested_xp, last_update, is_resting)
	 ...
	 if pc == AlltheGold.pc and realm == self.realm and not is_resting then
		  return rested_xp
	 end
...
do -- Time Played functions closure
	...
	-- Time Played display function that is hooked to prevent chat spam from AlltheGold
	...
	function AlltheGold:RequestTimePlayed()
...
function AlltheGold:FormatTime(seconds)
...
function FormatJustice( justice_points )
	if AlltheGold:GetOption('use_icons') then
...
function FormatHonor( faction, honor_kills, honor_points, arena_points, conquest_points )
	...
	if AlltheGold:GetOption('show_honor_kills') and honor_kills ~= nil then
		honor_string = honor_string .. fmt.hk:format(C:White(tostring(honor_kills))) .. '  '
	end
	if AlltheGold:GetOption('show_honor_points') and honor_points ~= nil 		then
...
function ClassColour( class, faction, string )
	 if class == "" or not AlltheGold:GetOption('colorize_class') then
...
function FormatCharacterName( pc, level, xp, seconds_played, class, class_loc, faction, last_update )
	--AlltheGold:Debug("FormatCharacterName: %s, %s, %s, %s, %s, %s, %s",pc, level, xp, seconds_played, class, class_loc, faction)

	local result_string     = ""
	local level_string      = ""

	-- Format the level string according to the show_progress option
	if AlltheGold:GetOption('show_progress') and xp ~= -1 then
		local progress
		if level == AlltheGold.max_pc_level then
			progress = 0
		else
	...
	if class_display ~= "" and AlltheGold:GetOption('show_class_name') then
		level_string = ("%s %s"):format( class_display, level_string )
	end

	result_string = ClassColour( class, faction, "%s (%s)" ):format( pc, level_string )

	if AlltheGold:GetOption('show_played_time') then
		result_string = result_string .. FactionColour(faction, " : %s"):format(AlltheGold:FormatTime(seconds_played))
	end

	-- if AlltheGold:GetOption('show_last_login') then
	-- 	if pc == AlltheGold.pc then
	-- 		-- The PC is loged in
	-- 		result_string = result_string .. FactionColour(faction, " : *** %s ***"):format(L["Logged In"])

	-- 	else
	-- 		result_string = result_string .. FactionColour(faction, " : Seen %s ago"):format(AlltheGold:FormatTime(time() - last_update))
	-- 	end
	-- end

	-- Do we need to show the total XP
	if AlltheGold:GetOption('show_xp_total') and xp ~= -1 then
		local pc_xp = XPToLevel(level)
		if AlltheGold.max_pc_level < level then pc_xp = pc_xp + xp end
...
function AlltheGold:BuildSortTables()
...
do
	...
	function buildSortedTable( unsorted_table, sort_function, realm )
		--AlltheGold:Debug("buildSortedTable:")
	...
	function PCSortByRestedXP(a,b)
		 ...
		 if a and table_to_sort[a] then
			  estimated_rested_xp_a = AlltheGold:EstimateRestedXP(
											a,
											realm_for_sort,
		 ...
		 if b and table_to_sort[b] then
			  estimated_rested_xp_b = AlltheGold:EstimateRestedXP(
											b,
											realm_for_sort,
		 ...
		 --AlltheGold:Debug("PCSortByRestedXP: %s = %s, %s = %s",a, estimated_rested_xp_a, b, estimated_rested_xp_b)
	...
	function PCSortByPercentRest(a,b)
		 ...
		 --AlltheGold:Debug("PCSortByPercentRest: %s = %s, %s = %s",a, estimated_rested_xp_a, b, estimated_rested_xp_b)
...
function InitXPToLevelCache( game_version, build_version )
	...
	if AlltheGold.db.global.cache.XPToNextLevel[build_version] ~= nil then
		for level = 1, AlltheGold.max_pc_level do
			if AlltheGold.db.global.cache.XPToNextLevel[build_version][level] ~= nil and
				AlltheGold.db.global.cache.XPToNextLevel[build_version][level] ~= 0 then
				XPToNextLevelCache[level] = AlltheGold.db.global.cache.XPToNextLevel[build_version][level]
			end
			-- Remove entries that may have been inserted due to a bug
			if AlltheGold.db.global.cache.XPToNextLevel[build_version][level] == 0 then
				AlltheGold.db.global.cache.XPToNextLevel[build_version][level] = nil
			end
		end
	end
...
-- #################################################################################
-- ##
-- ## AlltheGold.GetClassHexColour(class)
-- ## ----------------------------------
-- ##
...
function AlltheGold.GetClassHexColour(class)
...
AlltheGoldLDB = ldb:NewDataObject("AlltheGold", {
	type = "data source",
	text = "***AlltheGold***",
...
function AlltheGoldLDB:OnClick(button,down)
...
function AlltheGoldLDB:OnEnter(motion)
	AlltheGold:DrawTooltip(self)

end

function AlltheGoldLDB:OnLeave()
	-- Nothing to so, the update frame take care of the tooltip disposal
end

--AlltheGoldLDB.tooltip = AlltheGold.tablet
